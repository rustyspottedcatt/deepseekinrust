...
fn tensor_to_device_buffer(tensor: &Tensor) -> Result<DeviceBuffer<f32>, Box<dyn std::error::Error>> {
    let slice: &[f32] = tensor.try_into_data::<f32>()?.as_slice();
...
pub fn act_quant(x: &Tensor) -> Result<(Tensor, Tensor), Box<dyn Error>> {
    ...
    let ptx_cstr = CString::new(include_str!("../cuda/act_quant_kernel.ptx"))?;
    ...
    unsafe {
        ...
        launch!(
        function<<<(x_numel as i32 / *BLOCK_SIZE) as u32, 1, 1, stream>>>(
            x.as_device_ptr(),
            y.as_device_ptr(),
            s.as_device_ptr(),
            BLOCK_SIZE
...
pub fn weight_dequant(x: &Tensor, s: &Tensor) -> Result<Tensor, Box<dyn Error>> {
    ...
    let ptx_cstr = CString::new(include_str!("../cuda/weight_dequant_kernel.ptx"))?;
    let module = Module::load_from_string(&ptx_cstr)?;
    ...
    unsafe {
        launch!(
             function<<<(m + *BLOCK_SIZE - 1) / *BLOCK_SIZE, (n + *BLOCK_SIZE - 1) / *BLOCK_SIZE, 1, stream>>>(
                x_dev.as_device_ptr(),
                s_dev.as_device_ptr(),
                y_dev.as_device_ptr(),
                m,
...
pub fn fp8_gemm(a: &Tensor, a_s: &Tensor, b: &Tensor, b_s: &Tensor) -> Result<Tensor, Box<dyn Error>> {
    ...
    let ptx_cstr = CString::new(include_str!("../cuda/fp8_gemm_kernel.ptx"))?;
    let module = Module::load_from_string(&ptx_cstr)?;
